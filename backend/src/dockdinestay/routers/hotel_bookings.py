# app/routers/hotel_bookings.py

from fastapi import APIRouter, Depends, HTTPException, status
from typing import List

from dockdinestay.db import HotelBooking, HotelBookingCRUD, UserRole
...
from dockdinestay.routers.dependencies import get_hotel_booking_crud
...
async def get_hotel_booking_by_id(
    ...
    if (
        ...
        == UserRole.CUSTOMER.value  # Corrected for string comparison
...
async def update_hotel_booking(
    ...
    if (
    ...
    ):  # Corrected for string comparison
        allowed_fields = {
            "notes",
            "status",
        }
        ...
        if (
            ...
            != current_booking.status.cancelled.value  # Compare enum values
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
